...
\vspace{-5pt}
\begin{center}
{\gt \Large J-PARC チャームバリオン分光実験のための散乱粒子多重度によるリアルタイム事象選別プロセスの性能評価 }\\[14pt]

{\gt \large 阪大RCNP$^A$, KEK$^B$, 東大CNS$^C$, 総研大$^D$ \\ 加島駿一$^A$, 白鳥昂太郎$^A$, 野海博之$^A$$^B$, 五十嵐洋一$^B$, 郡司卓$^C$, 鈴木謙$^A$, 本多良太郎$^B$, 古川史也$^A$, Che-sheng Lin$^D$, 他T103 コラボレーション, 他SPADI Alliance}\\[5pt]

% {\gt \large 日本科技大$^A$ 帝都大理工$^B$ \\ 止田次郎$^A$, 潟川　学$^B$}\\[5pt]

{\large \bf Performance evaluation of the real-time event selection process by a criterion based on scattered particle multiplicity for the J-PARC charmed baryon spectroscopy experiment}\\[5pt]

{\large \it RCNP$^A$, KEK\,$^B$, CNS\,$^C$, SOKENDAI\,$^D$}\\

{\large \bf S. Kashima$^A$, K. Shirotori$^A$, H. Noumi$^A$$^B$, Y. Igarashi$^B$, T. Gunji$^C$, K.
Suzuki$^A$, R. Honda$^B$, F. Furukawa$^A$, Che-sheng Lin$^D$, for T103 Collaboration, and for SPADI Alliance}\\
...
実験で想定される高い信号レートの環境下でチャームバリオン生成事象を効率的に収集するには複雑な事象選別が要求されるため、
% 実験で用いる汎用スペクトロメーター(MARQ)のデータ取得ではハードウェアトリガーを撤廃し、その性能に依存しない連続読み出しDAQを用いる。
我々はハードウェアトリガーを撤廃した連続読み出し式データ収集システムとソフトウェアによるオンラインフィルタープロセスを組み合わせたデータ取得システム(NestDAQ)を採用する。
連続読み出し式データ収集システムにおいて計算機ネットワークへ送られる無選別のデータから不要な事象を除去するためのオンラインフィルタープロセスの開発を行っている。

J-PARC K1.8BRに建設したテストベンチを用いて2024 年4--6 月のビームタイムでハドロンビームを用いた連続読み出し式DAQシステムの動作試験$^{\left[1\right]}$を実施した。そこで取得されたデータを用いて、ビーム粒子が標的と反応せずに通過する事象を除去するために、散乱粒子の多重度による一連のフィルターアルゴリズムを検討し、事象選別によるデータレート削減率の評価と必要な計算資源の調査を行った結果について報告する。
