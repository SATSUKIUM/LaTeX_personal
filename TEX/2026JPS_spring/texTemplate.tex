...
J-PARC K1.8BRに建設したテストベンチを用いて2024 年4--6 月のビームタイムでハドロンビームを用いた連続読み出しDAQの試験$^{\left[1\right]}$を実施した。そこで取得されたデータを用いて、ビーム粒子が標的と反応せずに通過する事象を除去するために、散乱粒子の存在度による一連のフィルターアルゴリズムを検討し、事象選別によるデータレート削減率の評価と必要な計算資源の調査を行った結果について報告する。
